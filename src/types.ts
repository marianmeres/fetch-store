 ...
 * @template T - The data type stored in the fetch store
 */
export interface FetchStore<T> extends StoreReadable<FetchStoreValue<T>> {
	/**
	 ...
	 */
	fetch: (...args: unknown[]) => Promise<T | null>;
	 ...
	 * @returns Promise resolving to fetched data or null on error
	 */
	fetchSilent: (...args: unknown[]) => Promise<T | null>;
	 ...
	 * @returns Promise resolving to fetched data or null on error
	 */
	fetchOnce: (args?: unknown[], thresholdMs?: number) => Promise<T | null>;
	 ...
	 * @returns Promise resolving to fetched data or null on error
	 */
	fetchOnceSilent: (args?: unknown[], thresholdMs?: number) => Promise<T | null>;
 ...
 */
export type FetchStreamEventEmitFn = (
 ...
 * @template T - The data type stored in the stream store
 */
export interface FetchStreamStore<T> extends StoreReadable<FetchStreamStoreValue<T>> {
	/**
